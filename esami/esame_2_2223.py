...
def func1(string_list1, string_list2):
    lista_da_tornare = []
    for elem1 in string_list1:
        if elem1 not in string_list2:
            lista_da_tornare.append(elem1)
    for elem2 in string_list2:
        if elem2 not in string_list1:
            lista_da_tornare.append(elem2)
    return sorted(lista_da_tornare, key=lambda x.lower(),reverse=True)
...
def func3(a_list):
    M = max(a_list)
    m = min(a_list)
    len_initial = len(a_list)
    a_list[:] = [n for n in a_list if n != M and n != m]

    len_final = len(a_list)
    
    return  len_initial - len_final
...
def ex2(root):
    return ex2a(root,0,0)
    def ex2a (node,level,count):
        if not root:
            return count
        if level %2:
            count += node.value
            ...
            count -= node.value
        count = ex2a(node.dx, level+1,count)
        count = ex2a(node.sx, level+1,count)    
        return count
