...
def func1(int_list, bottom, up):
    ...
    for intero in int_list:
        if (intero >= bottom and intero <= up):
           new_list.append(intero)
           counter += 1
...
def func2(dict1, dict2):
    ...
    for key , value in dict1.items():
        if key in dict2:
            if value < dict2[key]:
                dizio_dator[key]= value.lower()
            else:
                dizio_dator[key] = dict2[key].lower()
    return dizio_dator
...
def func3(str1, str2):
    ...
    if len(str1)>len(str2):
        for j,char in enumerate(str2):
            if char.lower() == str1[j].lower():
                str3=str3 + char
    if len(str1)<len(str2):
        for i,char in enumerate(str1):
            if char.lower() == str2[i].lower():
                str3=str3 + char     
    return str3
...
def func4(input_filename, output_filename, length):
    ...
    with open(input_filename,mode="r",encoding="utf-8")as f:
        lista_parole=f.read().strip().split()
    aus_var=[]
    for i, parola in enumerate(lista_parole):
        if len(parola) == length:
            aus_var.append(parola)
            counter+=1
    lista_parole[:] = sorted(aus_var, key=str.lower)

    aus_var = []
    riga = []
    for parola in lista_parole:
        if not riga or parola[0].lower() == riga[0][0].lower():
            riga.append(parola)
        else:
            aus_var.append(riga)
            riga = [parola]
    ...
    with open(output_filename,mode="w",encoding="utf-8")as fileOut:
        for riga in aus_var:
            fileOut.write(" ".join(riga) + "\n")    
    return counter      
...
"""
#caso base è quando n diventa 0

def ex1(n, faces):
    if n == 0:
        return [()]
    risultati_precedenti= ex1(n-1, faces)
    risultati = []
    for esito in risultati_precedenti:
        for faccia in range(1, faces + 1):
            risultati.append(esito + (faccia,))
...
"""
import tree
def change_explore(node,counter):
    if not node:
        return
    if node.sx and node.dx:
        if node.sx<node.dx:
            ...
            change_explore(node.dx, counter)
    else:
        change_explore(node.sx,counter)
        change_explore(node.dx, counter)
def ex2(root):
    counter=0
    change_explore(root,counter)
    return counter
