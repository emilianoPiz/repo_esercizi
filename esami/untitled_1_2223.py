...
def func1(string_list, word):
    string_list[:] =  [x for x in string_list if word not in x]    
...
def func3(listaA, pathname):
    stringa= ""
    counter=0
    listaA = sorted(listaA,key=lambda x: (len(x),x[::-1]))
    for elem in listaA:
        counter += len(elem)
        elem + "/n"        
        stringa += elem
...
def func4(S):
    isto=""
    lista_parole = S.strip().split()
    for i in range(len(lista_parole)):
        lista_parole[i] = lista_parole[i].lower()
    #cerco il max per capire quanto lungo deve esse l'istogramma 
    M = len(max(lista_parole,key=lambda x :len(x)))
    #conto occorrenze
    occorrenze= {key:0 for key in lista_parole}
    for parola in lista_parole:
        if parola in occorrenze:
            occorrenze[parola] +=1
    #rimuovo doppioni
    lista_parole = list(set(lista_parole))
    lista_parole = sorted(lista_parole,key=lambda x:x)
    for parola in lista_parole:
        k =  M-len(parola)
        times = occorrenze[parola]
        parola += " " * (k+1)
        parola += "*"*times+"\n"
        isto += parola
    return isto
